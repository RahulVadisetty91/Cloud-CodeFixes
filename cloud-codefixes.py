...
def test_parse_args_defaults_to_random_color(tmp_text_file):
    args, _, _ = cli.parse_args(['--text', str(tmp_text_file)])
...
def test_unicode_text_file():
    ...
    _, text, _ = cli.parse_args(['--text', unicode_file])
    assert len(text) == 16
...
def test_unicode_with_stopwords():
    ...
    _, _, image_file = cli.parse_args(['--text', unicode_file, '--stopwords', stopwords_file])
...
def test_cli_writes_to_imagefile(tmpdir, tmp_text_file):
    ...
    args, text, _ = cli.parse_args(['--text', str(tmp_text_file), '--imagefile', str(tmp_image_file)])
